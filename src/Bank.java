...
// It returns true if the customer was added successfully or false otherwise.
    public boolean addCustomer(String branchName, String customerName, double initialTransaction) {
        Branch tempBranch = findBranch(branchName);
        if (tempBranch != null && customerName != null) return tempBranch.newCustomer(customerName, initialTransaction);
        return false;
    }
...
// It returns true if the customers transaction was added successfully or false otherwise.
    public boolean addCustomerTransaction(String branchName, String customerName, double transaction) {
        Branch tempBranch = findBranch(branchName);
        if (tempBranch != null) return tempBranch.addCustomerTransaction(customerName, transaction);
        return false;
    }
...
// This method prints out a list of customers.
    public boolean listCustomers(String branchName, boolean printTransactions) {
        Branch tempBranch = findBranch(branchName);
        if (tempBranch != null) {
            tempBranch.getCustomers();
            if (printTransactions) for (int i = 0; i < tempBranch.getCustomers().size(); i++) {
                tempBranch.getCustomers().get(i).getTransactions();
            }
            return true;
        }
        return false;
